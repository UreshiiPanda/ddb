"""
    This Flask/Python file handles route for "User_Accounts" table in 
    the DB CRUD Functionality 
...
def index():
    """
    Handles homepage (User_Accounts Table)
        ...
        try:
            ...
            if len(req) > 0:
                ...
                # Otherwise payload must be from the INSERT username button:
                    ...
                    # Send feedback to frontend with flash()
                    flash(f"User Account '{req['username']}' added", "info")
                return "OK", 202  # static/UI_logic.js is handling the page reload
